...
//send email to users who have created accounts
func sendEmail(email, firstName, lastName, mobile string){
	//set up stmp auth
	auth:= stmp.PlainAuth("", os.Getenv("STMP_EMAIL"), os.Getenv("STMP_PASSWORD"), os.Getenv("STMP_HOST"))
	addr:= os.Getenv("STMP_PASSWORD")

	//set up email details
	from := "kevinmranda042@gmail.com"
	to := []string{email}
	subject := "Successful Account Creation"
	body := fmt.Sprintf("Thank you, %v %v for creating an account at CLICKM")
