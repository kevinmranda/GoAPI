...
func AzamPayCallback(accountNumber, amount, message, utilityRef, operator, reference, transactionStatus, submerchantAcc, fspReferenceId string) {
	// Create JSON body
	jsonBody := fmt.Sprintf(`{
		"additionalProperties": {
			"property1": null,
		...
		"submerchantAcc": "%s",
		"fspReferenceId": "%s"
		}`, accountNumber, amount, message, utilityRef, operator, reference, transactionStatus, submerchantAcc, fspReferenceId)
...
func AzamPayCallbackHandler(c *gin.Context) {
	...
	// Call the actual AzamPayCallback function from the controller
	// AzamPayCallback(
	// 	body.AccountNumber,
	// 	body.Amount,
	// 	body.Message,
	...
	// Send a response to indicate the callback was processed
