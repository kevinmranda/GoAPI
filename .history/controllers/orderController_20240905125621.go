...
func GetOrders(c *gin.Context) {
	...
	result := initializers.DB.
		...
		Preload("Photos", "user_id = ?", id).
		Preload("Payment").
		Group("orders.id, customers.customer_email").                   // Include customer_email in the GROUP BY clause
		Select("orders.*, customers.customer_email AS customer_email"). // Select customer email
		Find(&orders)
