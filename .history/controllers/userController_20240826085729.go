...
func SendResetPasswordEmail(c *gin.Context) {
	...
	// Email body template
	body := fmt.Sprintf("Click the link to reset your password: http://localhost:4200/resetPassword?token=%s", tokenString)
...
func ResetPassword(c *gin.Context) {
	...
	initializers.DB.First(&token, "token = ?", body.Token)
