...
func AuthToken() (string, bool) {
	...
	return authResp.Data.AccessToken, authResp.Success
}

func MNOCheckout(accountNumber, amount string) (bool, string) {
	// Generate the token
	token, success := AuthToken()
	...
	jsonBody := fmt.Sprintf(`{
		...
		"amount": "%s",
		"currency": "%s",
		"externaId": "h87yh8hu874r",
		"provider": "%s"
	}`, accountNumber, amount, currency, network)
...
func GetMNO(phoneNumber string) string {
	...
	mnoPrefixes := map[string][]string{
		"Vodacom": {"0754", "0755", "0756", "0757", "0758", "0767", "0742", "0768"},
