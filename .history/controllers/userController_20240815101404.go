...
func validateUserInput(firstName, lastName, email, mobile, password string) (bool, bool, bool) {
	isValidName := len(firstName) > 0 && len(lastName) > 0
	isValidEmail := validateEmail(email)
	isValidPassword := validatePassword(password)
	isValidPhoneNumber := validatePhoneNumber(mobile)

	return isValidName, isValidEmail, isValidPassword, is
}
...
func validatePhoneNumber(phoneNumber string) bool {
	...
	re := regexp.MustCompile(`^(?:\+?255|0|255)?\d{9}$`)
