...
func UpdateOrder(c *gin.Context) {
	...
	if contentForUpdate.Title != "" {
		order.Title = contentForUpdate.Title
	}

	if contentForUpdate.Description != "" {
		order.Description = contentForUpdate.Description
	}

	if contentForUpdate.Filename != "" {
		order.Filename = contentForUpdate.Filename
	}

	if contentForUpdate.Price != 0 {
		order.Price = contentForUpdate.Price
	}

	if contentForUpdate.Uploaded_by != 0 {
		order.User_id = contentForUpdate.Uploaded_by
	}
