...
func UpdatePayment(c *gin.Context) {
	...
	var payment models.Payment
	result := initializers.DB.Preload("Photos").Preload("Payment").First(&payment, id)
	...
	if contentForUpdate.OrderID != "" {
		payment.OrderID = contentForUpdate.OrderID
	}

	if contentForUpdate.Total_amount != 0 {
		payment.Total_amount = contentForUpdate.Total_amount
	}
