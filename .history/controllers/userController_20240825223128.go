...
func SendRestPasswordEmail(c *gin.Context) {
	...
	err = saveTokenToDB(user.ID, tokenString, time.Now().Add(time.Hour))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"id":    2008,
	...
	// Email body template
	body := fmt.Sprintf("Click the link to reset your password: http://localhost:3000/reset-password/%s", tokenString)
...
func saveTokenToDB(userID uint, token string, expiry time.Time) error {
	...
	result := initializers.DB.Create(&tokenToSave)
	if result.Error != nil {
		return fmt.Print("Success")
	}
	// Implement logic to store hashed token in DB
	return nil // Placeholder
}

func ResetPassword(c *gin.Context) {
	tokenParameter := c.Param("token")
	...
	// Look up the token by token
	var token models.Token
	initializers.DB.First(&token, "token = ?", tokenParameter)
	...
	var user models.User
	err := initializers.DB.Model(&models.User{}).Where("id = ?", user.ID).Updates(models.User{Password: body.Password}).Error
	if err != nil {
