...
func UpdateUser(c *gin.Context) {
	...
	//update user with struct provided on request body
	user := models.User{
		First_name:       contentForUpdate.First_name,
		Last_name: contentForUpdate.Last_name,
		Gender:    contentForUpdate.Gender,
		Birthdate:       contentForUpdate.Birthdate,
		Address:     contentForUpdate.Address,
		User_id:     contentForUpdate.Address,
		User_id:     contentForUpdate.Mobile,
	}

	result = initializers.DB.Updates(&user)
	if result.Error != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"id":    2014,
